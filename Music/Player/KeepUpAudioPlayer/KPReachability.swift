//
//  AudioPlayerReachability.swift
//  Music
//
//  Created by Aleksandr on 13.02.2018.
...
class KPReachability {

    var changed: ((Bool) -> Swift.Void)?
    ...
    let reachabilitySerialQueue = DispatchQueue(label: "APReachabilityQueue")
...
extension KPReachability {
    ...
    func start() throws {
        ...
        var context = SCNetworkReachabilityContext(version: 0,
                                                   ...
                                                   release: nil,
                                                   copyDescription: nil)
        context.info = Unmanaged<KPReachability>.passUnretained(self).toOpaque()
    ...
    /** The specified node name or address can be reached using the current network configuration,
        ...
        the kSCNetworkReachabilityFlagsConnectionOnTraffic flag, kSCNetworkReachabilityFlagsConnectionOnDemand flag,
...
func callout(reachability: SCNetworkReachability, flags: SCNetworkReachabilityFlags, info: UnsafeMutableRawPointer?) {
    ...
    DispatchQueue.main.async {
        Unmanaged<KPReachability>.fromOpaque(info).takeUnretainedValue().flagsChanged()
    }
}

struct Network {
    static var reachability: KPReachability?
